...
class chiplet:
    ...
    def __init__(self, code_dist, defect_list=[]):
        self.d = code_dist
        ...
        if defect_list:
            self.set_defects(defect_list)

    def set_defects(self, defect_list):
        for x, y in defect_list:
            if x % 2 == 0:  # data qubit
    ...
    def broken_links(self, link_err):
        ...
        for i in range(self.dataq_lattice.shape[0]):
            for j in range(self.dataq_lattice.shape[1]):
                ...
                if (
                    j != self.dataq_lattice.shape[1] - 1
                    and i != self.dataq_lattice.shape[0] - 1
                ):
    ...
    def set_err_fixed_freq(self, link_err):
        broken_links = self.broken_links(link_err)
        # disable the data qubit included in each broken link
        for data_coords, syn_coords in broken_links:
...
class combined_chip:  # a combined chip consisting of an array of chiplets
    def __init__(self, code_dist, numq_row, numq_col):
        ...
        self.chiplet_array = [
            [chiplet(code_dist) for i in range(numq_col)] for j in range(numq_row)
        ]
    ...
    def check_middle_data(self, ta, tb):
        ...
        if ta[0] == tb[0] and abs(ta[1] - tb[1]) == 1:  # adjacent tiles in the same row
            ...
            if (
                self.chiplet_array[r][c].right_edge_defect()
                or self.chiplet_array[r][c + 1].left_edge_defect()
            ):
                return True
            else:
                return False
        elif (
            ta[1] == tb[1] and abs(ta[0] - tb[0]) == 1
        ):  # adjacent tiles in the same col
            r = min(ta[0], tb[0])
            c = ta[1]
            # coords are (r,c) and (r+1,c)
            if (
                self.chiplet_array[r][c].lower_edge_defect()
                or self.chiplet_array[r + 1][c].upper_edge_defect()
            ):
                return True
            else:
                return False
